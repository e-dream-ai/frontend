...
export const WebClientProvider: React.FC<{
  ...
  // overlay
  const { showOverlay, hideOverlay } = useVideoJSOverlay(
    playerInstance?.player ? [playerInstance.player] : [],
  );
  ...
  const defaultPlaylistDreams = useMemo(
    () => defaultPlaylistData?.data?.dreams ?? [],
    [defaultPlaylistData],
  );
  const dislikedDreams = useMemo(
    () => dislikesData?.data?.dislikes ?? [],
    [dislikesData],
  );
  ...
  const updateCreditOverlay = useCallback(() => {
    ...
    const { name: playlistName = "", uuid: playlistUuid = "" } =
  ...
  const addDreamToHistory = useCallback(
    (dream: Dream, wasConcatenated: boolean) => {
   ...
   */
  const preloadNavigationVideos = useCallback(
    async (navigation: PlaylistNavigation) => {
  ...
  const playDreamWithHistory = useCallback(
    async (dream?: Dream) => {
      if (!dream) return;
      ...
      if (added) {
        ...
        playlistHistoryPositionRef.current = Math.max(
          historyRef.current.findIndex((h) => h.dream.uuid === dream.uuid),
          0,
        );
      }
  ...
  const setSpeed = useCallback(
    (speed: number) => {
      const playbackRate = calculatePlaybackRateFromSpeed(
        speed,
        playingDreamRef?.current?.activityLevel,
      );
  ...
  > = useMemo(
    () => ({
      ...
      dislike_current_dream: async () => {
        ...
        const dislikedDreamIndex = historyRef.current.findIndex(
          (h) => h.dream.uuid === dislikedDreamUUID,
        );
  ...
  useEffect(() => {
    if (
      ...
      const defaultPlaylistDream = defaultPlaylistDreams.find(
        (d) => !dislikedDreamsRef.current.includes(d.uuid),
      );
  ...
  useEffect(() => {
    historyRef.current = historyRef.current.filter(
      (h) => !dislikedDreams.includes(h.dream.uuid),
    );
  }, [dislikedDreams]);
  ...
  return (
    <WebClientContext.Provider value={memoedValue}>
      {children}
